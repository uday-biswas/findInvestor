...
const AddToList: React.FC<AddToListProps> = ({ ids, page }) => {
    ...
    const email = useSelector((store: RootState) => store.profile.user.email);
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="sm:max-w-[425px]">
                ...
                <DialogFooter>
                    <Button onClick={handleSubmit} disabled={loading} className="rounded-[5px]">Save to List(s)</Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    )
